...
def read_user_keyword(uid):
    value = cur.execute(
        "select word from agora_userkeyword as au inner JOIN agora_keyword as ak on (au.keyword_pk_id=ak.id) where au.user_pk_id="+uid)
    user_keyword = "Act"  # 받아오기
    return user_keyword
...
def Id_recommendlist(uid):
    df_rating = pd.read_csv(dir_path + '/ratings.csv')
    # df_rating=read_room_score()
    df_rating = pd.DataFrame(df_rating)
    df_rating.columns = ['userId', 'classId', 'rating', 'date']
    ...
    df_rating = df_rating.head(100000)
    ...
    # 콜드스타트 경우 평점이 높은 스터디 추천 + 사용자의 목표,선호하는 keyword에 해당하는 스터디가 있으면 해당 스터디를 우선추천
        ...
        user_keyword = read_user_keyword()
