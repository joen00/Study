...
cur = conn.cursor()
cur.execute("SELECT ROOM,TITLE,MAIN_GOAL,SUB_GOAL FROM ROOM_INFO")
df_class = cur.fetchall() 

conn_string = "host = 'localhost' dbname = 'test_db' user = 'evergreen' password = 'evergreen'"
conn = psycopg2.connect(conn_string)
cur = conn.cursor()
cur.execute("SELECT USER,ROOM,SCORE FROM ROOM_SCORE")
df_rating=cur.fetchall() 

conn_string = "host = 'localhost' dbname = 'test_db' user = 'evergreen' password = 'evergreen'"
conn = psycopg2.connect(conn_string)
cur = conn.cursor()
cur.execute("SELECT USER_GOAL FROM USER WHERE PK=1234 ")
...
def recommend_class(df_svd_preds,  class_df, ratings_df, user_id, num_recommendations):
    ...
    user_history = pd.merge(user_data, class_df, on='classId').sort_values(
...
user_keyword = "A"
newuser_recommendlist = pd.DataFrame(index=range(0, 5), columns=[
                                     'classId', 'title', 'goal'])
if rating_count.iloc[uid-1]['count'] <= 1:  # 새로운 사용자임을 확인
    ...
    for i in range(0, class_count+1):
        ...
        if list_count < 5:
            df_sorted_by_values = pd.DataFrame(index=range(0, 5), columns=[
                                               'classId', 'title', 'goal'])
            # 평점순으로 배열한 클래스 5개 newuser_recommendlist에 추가하기
            df_sorted_by_values = df_class.sort_values(
                ['rating'], ascending=False)
        # newuser_recommendlist 5개로 자르기
        result = pd.concat(
            [newuser_recommendlist, df_sorted_by_values], ignore_index=True)
        newuser_result = result.iloc[[0, 1, 2, 3, 4]]
# 기존 사용자들의 스터디 추천
else:
    recommend_list = recommend_class(
        pred, df_class, df_rating, uid, recommendN)

    reflect_goal = recommend_list.reset_index(
        drop=True)[["classId", "title", "Predictions", "goal"]]
